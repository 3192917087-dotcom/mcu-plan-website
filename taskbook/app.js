...
function buildUserMessage({ topic, devices, funcs, refs, template, skeletonText }) {
  ...
  if (refs.length) {
    ...
    // 用户没有填参考文献：不写参考文献章节，不放占位符，不编造学者/论文
    // 国内外研究现状按通用技术路线描述即可，不着重引用
    parts.push(`【参考文献】用户未提供。请严格遵守：

1. 无论是默认 11 章框架还是模板骨架，"参考文献"相关章节全部跳过不写。
2. 绝不输出"[1]-[10] 待补充真实文献"或类似占位字符串。
3. 国内外研究现状按通用技术路线描述，不编造学者姓名/论文标题/期刊名。
4. 用户后续自己补真实文献，正文里不留任何"待补/占位"提示字符。`);
  }
