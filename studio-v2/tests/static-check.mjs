...
const assertions = [
  ...
  ['正文最低目标调整为15000字', app.includes('const MIN_BODY_CHARS = 15000') && prompts.includes('export const MIN_BODY_CHARS = 15000') && /id="paper-target-chars"[^>]*min="15000"[^>]*value="15000"/.test(html) && html.includes('表格、图位和Mermaid代码不计入系统有效字数')],
  ['第四章流程图按独立逻辑链自适应', prompts.includes('groupWorkflowFunctions') && prompts.includes('依据独立判断链决定核心流程图数量') && prompts.includes('不设固定数量') && app.includes('应恰好归入一条独立程序逻辑链') && !app.includes('核心功能流程图超过2张')],
  ...
  ['励志语有本地回退并定期调用AI补充', html.includes('id="daily-motivation-text"') && app.includes('function refreshDailyMotivation') && app.includes('MOTIVATION_AI_INTERVAL = 4') && prompts.includes('buildDailyMotivationMessages')],
  ['励志语按随机间隔刷新并横向滚动', app.includes('MOTIVATION_REFRESH_MIN_MS = 90 * 1000') && app.includes('MOTIVATION_REFRESH_MAX_MS = 4 * 60 * 1000') && app.includes('function randomMotivationDelay') && app.includes('nextRefreshAt: updatedAt + randomMotivationDelay()') && app.includes('restartMotivationTicker') && html.includes('daily-motivation-viewport') && styles.includes('daily-motivation-marquee')],
  ['励志语按绝对时间补更新并迁移旧缓存', app.includes('function motivationRefreshDue') && app.includes('MOTIVATION_SCHEDULE_VERSION = 2') && app.includes('needsScheduleMigration') && app.includes("document.visibilityState === 'visible'") && app.includes("window.addEventListener('focus'")],
  ...
  ['系统框架与软件模块图职责分离', prompts.includes('系统软件功能模块图') && prompts.includes('用5至9个节点表达主程序') && prompts.includes('程序任务划分与调用层级') && prompts.includes('不得照搬第二章系统总体功能框架图') && app.includes('系统总体功能框架图使用flowchart LR表达硬件组成及信息流')],
  ['下载页准确说明WPS样式绑定', html.includes('正文、标题 1/2/3、题注、表格文字') && html.includes('修改对应样式即可统一更新同类内容格式')],
  ['时序图固定横向绘制', prompts.includes('时序图必须使用flowchart LR') && prompts.includes('禁止使用sequenceDiagram') && app.includes('必须使用flowchart LR按时间从左向右横向绘制') && app.includes('severity: \'blocking\'')],
