...
async function main() {
  ...
  if (!/<w:style[^>]+w:styleId="Normal"[\s\S]*?<w:name w:val="正文"\/>/.test(stylesXml)) {
    throw new Error('DOCX没有建立可供WPS修改的正文样式');
  }
  const requiredStyles = [
    ['Heading1', '标题 1'], ['Heading2', '标题 2'], ['Heading3', '标题 3'], ['Caption', '题注'],
    ['ThesisTableHeader', '论文表头文字'], ['ThesisTableText', '论文表格文字'], ['ThesisReference', '论文参考文献'],
  ];
  ...
  const bodyParagraphProperties = [...documentXml.matchAll(/<w:pPr><w:pStyle w:val="Normal"\/>[\s\S]*?<\/w:pPr>/g)].map(match => match[0]);
  ...
  ['Heading1', 'Heading2', 'Heading3', 'Caption', 'ThesisTableHeader', 'ThesisTableText'].forEach(id => {
  ...
  const splitParagraphs = [...documentXml.matchAll(/<w:p><w:pPr><w:pStyle w:val="Normal"\/>[\s\S]*?<\/w:p>/g)].filter(match => match[0].includes('分段验证'));
