...
export function buildArtifactPlan({ outline = [], devices = [], functions = [] } = {}) {
  ...
  const artifacts = [];
  const contentDevices = devices.filter(device => !/主控|单片机|电源|晶振|复位|下载|调试/.test(`${device.role || ''} ${device.model || ''}`));
  if (overall) {
    artifacts.push(makeArtifact('system-framework', overall, '系统总体功能框架图', '正文先说明该图用于表达系统功能层次。插图说明必须列出每个方框名称、方框分层、信号流向和箭头文字：输入层在左、主控层居中、显示/执行/通信层在右，电源作为公共支撑放在下方；不写具体引脚，不与硬件组成图混淆。', '', { sectionId: '2.2', reason: '说明系统各功能模块之间的总体关系' }));
    ...
    contentDevices.forEach(device => {
      const name = device.model || device.name;
      artifacts.push(makeArtifact('device-image', overall, `${name}器件图`, `写明插在${name}器件介绍和选型结论之后。说明应插入器件正面清晰图片或实物近景，完整显示器件外形、排针方向和型号丝印；背景简洁、画面端正，不使用无关宣传图。给出规范图题，并说明正文中只首次引用一次。`, '', { sectionId: '2.4', sourceFactIds: [device.id], reason: '帮助读者识别实际选用器件' }));
    });
  }
  if (hardware) {
    artifacts.push(makeArtifact('hardware-block', hardware, '系统硬件组成图', '明确画出主控、每类外设、供电模块和主要接口类型。说明各模块在图中的位置、连接线方向、I2C/UART/SPI/GPIO等线旁标注以及供电关系；该图表达物理组成，不重复总体功能框架图。', '', { sectionId: '3.1', reason: '说明系统硬件的物理组成' }));
    if (contentDevices.length) artifacts.push(makeArtifact('pin-table', hardware, '主要外设引脚连接关系表', '放在各电路详细说明之前，按“外设、外设信号、主控引脚、信号方向、说明”组织简短三线表；每表最多10行，器件较多时按采集、执行、显示通信拆表。表格只概括连接，电气原因和驱动关系留在对应正文说明。', '', { sectionId: '3.1', sourceFactIds: contentDevices.map(device => device.id), reason: '集中核对已确认引脚并减少正文重复' }));
    contentDevices.forEach(device => {
      const name = device.model || device.name;
      artifacts.push(makeArtifact('circuit', hardware, `${name}接口电路图`, `写明插在${name}连接关系说明之后。绘图说明必须逐项列出主控端引脚、器件端信号、供电电压、共地、上拉/下拉、电阻或驱动器件，以及信号方向；图中网络名与已确认引脚完全一致，并给出图题和建议绘图范围。`, '', { sourceFactIds: [device.id], reason: '对应说明该器件与主控的实际电气连接' }));
    });
  }
  if (software) {
    artifacts.push(makeArtifact('software-architecture', software, '系统软件结构图', '写明主程序、初始化、数据采集、状态判断、执行控制、显示、通信和异常处理模块之间的调用关系。说明每个框的名称、上下层级和箭头方向；只表达软件任务关系，不出现硬件接线。', '', { sectionId: '4.1', reason: '说明软件模块和数据流之间的总体关系' }));
    artifacts.push(makeArtifact('flowchart', software, '主程序流程图', '直接给出简洁、可复制的Mermaid代码，使用flowchart TD。控制在6至10个节点，仅保留初始化、采集、判断、执行、显示/通信更新、异常处理和循环等关键步骤；判断分支使用“是/否”短标签，不使用subgraph、style、HTML或冗长节点文字。', '主程序', { sectionId: '4.2', reason: '说明系统软件的主执行顺序' }));
    distinctFunctions.forEach(func => {
      const name = functionSource(func);
      artifacts.push(makeArtifact('flowchart', software, `${name}流程图`, `直接给出简洁、可复制的Mermaid代码，使用flowchart TD。用5至8个节点表达${name}的输入、处理、关键判断、正常动作、异常处理和返回；判断分支使用“是/否”短标签，不使用subgraph、style、HTML或重复节点。`, name, { sourceFactIds: [func.id], reason: '确保每项确认功能都有可直接绘制的软件流程' }));
      if (/温湿度|单总线|DHT|DS18|I2C|SPI|UART|串口|通信|无线|蓝牙|WiFi|射频/i.test(name)) {
        artifacts.push(makeArtifact('timing', software, `${name}通信时序图`, '说明横轴时间、每条信号线名称、空闲电平、起始条件、应答、各数据阶段、采样边沿、超时条件和结束条件；逐段写出绘制顺序及关键时间参数，保证读者能够按文字重新绘制。若确实没有严格时序，则明确说明不单独设置时序图。', name, { sourceFactIds: [func.id], reason: '说明该功能中通信或严格时序的执行关系' }));
      }
    });
  ...
  if (test) {
    ...
    distinctFunctions.forEach(func => {
      const name = functionSource(func);
      artifacts.push(makeArtifact('result-image', test, `${name}功能展示图`, `写明插在${name}操作步骤和测试结果之后。拍摄/截图说明必须包括：系统所处测试条件、用户执行的操作、画面中必须出现的实物或界面、需要清晰展示的数值/状态/指示灯/执行结果、建议取景范围和图题；正文只首次引用一次，并在同一段分析观察结果。`, name, { sourceFactIds: [func.id], reason: '展示该功能在实际操作后的可观察结果' }));
    });
  }
...
export function buildSchemeMaterialImportMessages({ rawText, currentTitle = '' }) {
  return [
    {
      role: 'system',
      content: `你是单片机论文项目资料整理员。请从用户粘贴或导入的方案中识别论文题目、器件清单、功能要求和可用于论文写作的补充说明。必须忠实保留方案中明确写出的型号和功能，不要改换主控，不要凭空删除内容；只有缺少作用说明时才根据型号补充简短作用。把同一器件去重，把每项功能整理成一行。任何ESP8266字样统一规范为ESP-01S；0.96寸OLED不得误写成96寸OLED。
...
export function buildHardwareMessages(materials) {
  return [
    {
      role: 'system',
      content: `你是单片机项目硬件事实整理工程师。用户填写内容优先，不能擅自替换。资料只有题目时给出一套常见、真实、适合本科制作的建议；资料部分缺失时只补缺失项。用户不需要预先填写硬件连接关系，你必须先根据器件型号识别通信方式和所需信号，再依据主控资源生成供用户下拉确认的引脚建议。必须识别准确主控型号、外设型号、接口类型和每个接口需要的信号。不要把VCC、GND放入GPIO分配，但必须在powerNotes中说明供电电压和共地关系。需要独立WiFi模块时使用ESP-01S，禁止输出ESP8266。不要声称项目无法完成。
...
目录必须包含6章：第1章绪论、第2章系统总体方案设计、第3章系统硬件设计、第4章系统软件设计、第5章系统调试与功能测试、第6章总结与展望。目录详略必须服从目标正文总字数：每个三级标题都应当能够支撑一段完整的实质内容，不得为了显得详细而过度拆分；器件较多或功能较多时可适当增加对应三级标题，但同类型内容应归纳。每章必须有二级标题，第1至第5章必须包含有实际内容的三级标题：
...
export function chapterResponsibilities(kind) {
  const map = {
    ...
    hardware: '只写主控、电源和各外设电路及实际连接关系。按照目录三级标题逐项说明信号、引脚、供电、共地、阻容或驱动，可用简短引脚表和电路图；不得重复第二章的器件参数、候选比较和选型理由，也不能提前写程序流程。',
    software: '只写软件结构、主程序、器件驱动过程和各核心功能逻辑。每项确认功能必须有简洁、可复制的Mermaid流程图；通信或严格时序可用时序图，真实计算才使用公式。不得粘贴程序源代码，不直接用函数名堆叠介绍，也不得重复硬件接线。',
...
3. 标题必须单独成行且不使用Markdown井号。二级标题严格写成“x.x 标题”，三级标题严格写成“x.x.x 标题”。必须完整保留requiredSections给出的全部二级、三级标题，并按顺序写作；三级标题数量由项目器件、功能和自动规划目录决定，不得为了缩短目录擅自合并或删除。确有必要时可以增加三级标题，但不能建立没有实质内容的空标题。
4. 不插入源代码，不用具体函数名作为主体介绍。
5. 每个chapterArtifacts项目都必须在对应内容之后出现，不能遗漏。图片、框架图、电路图、时序图和展示图使用独占一行的“【非正文·图x-x待插入：图名】”，随后分别写清插入位置、必须展示的对象/状态/标注、拍摄或绘制方法、建议取景范围和规范图题；流程图改用第7条规定的Mermaid代码，不再写逐节点绘制说明。电路图还要逐项写清主控引脚、外设端信号、供电、共地、阻容/驱动和信号方向。每条说明都用独立的“【非正文·……】”行，最后单独写“【非正文结束】”。comparison-table、pin-table和test-table必须直接生成可用的Markdown表格，不得只预留表格位置；formula必须直接写出公式并解释变量、单位、参数来源和用途，不得将公式写成图片占位。非正文提示前后各空一行，绝不能接在正文句末。图形首次引用必须融入有分析内容的正文句子，禁止把“如图x-x所示。”单独成段，同一图只引用一次。
6. 系统功能框架图、硬件组成图、电路图、软件结构图和程序流程图必须名称与内容明确区分。
7. 主程序和每个核心功能流程图都必须直接给出简洁、可复制的Mermaid代码。固定格式为独占一行的“【非正文·Mermaid流程图：图名】”，下一行用三个反引号加mermaid开启代码围栏，首行必须是flowchart TD，末尾用三个反引号关闭代码围栏，再单独写“【非正文结束】”。主流程控制在6至10个节点，单项功能控制在5至8个节点；节点文字使用简短中文，判断分支只用“是/否”等短标签。禁止subgraph、style、classDef、HTML标签、重复节点和为了显得详细而堆叠步骤。
8. 时序图只用于确有通信或严格时序的功能。公式只用于真实计算，必须解释变量、单位、参数来源和用途。
9. 表格使用标准Markdown表格，表题“表x-x 表名”单独一行并放在表格之前。表格最多5列、10行；内容较多时按功能或模块拆表。表格前后必须有分析，不能只放表格。导出端会统一转换为三线表，不要在表格中模拟竖线装饰或合并单元格。
...
12. 目标有效正文约${target}个中文字符，不用标题、表格、图位和非正文说明凑字数。`;
  const userPayload = {
    title: project.title,
...
export function buildExtrasMessages(project) {
  ...
  return [
    {
      role: 'system',
      content: `根据论文完整正文生成英文论文题目、中文摘要、英文摘要、中英文关键词和致谢。英文题目必须准确对应中文题目，采用论文标题式自然表达，不添加中文题目中没有的创新或功能。摘要不是章节目录复述，应以连续、凝练的方式概括研究背景与目的、总体设计方法、硬件与软件实现思路、实现的主要功能、验证方式和总体结论。

摘要绝对禁止出现具体器件型号、芯片料号、引脚、寄存器、函数名、阈值、测试次数以及带单位的具体测试数据；STM32F103C8T6、DHT11、ESP-01S等只能概括为“微控制器、传感器、无线通信模块”等类别。不得引用参考文献，不写图表编号，不夸大创新性，不出现“尚未实现、未完成、受条件限制”等表述。中文摘要约500至700字；英文摘要与中文含义一致并遵守相同禁令。
...
export function buildAuditMessages(project) {
  const payload = {
    ...
    checks: [
      ...
      '每个图片预留均明确插入位置、画面内容、拍摄/绘制方法、取景或标注要求和图题',
      '图表预留及说明单独成行，表题在表格之前且表格可转换为三线表',
      '不存在系统未完成或功能缺失式自曝',
      '摘要没有具体器件型号、引脚、阈值或测试数据，只对全文进行概述',
...
跨章重复必须标为blocking，并指出前后章节；能够安全修复时，find必须是待修改章节中唯一出现的完整原文片段，replace应删除前章已经讲过的参数、原理、接线或程序步骤，改成本章职责专属的分析，而不是同义改写。硬件矛盾只能以用户确认事实为准修复。禁止修改确认的型号、引脚和功能，禁止整章重写。无法安全自动修复时repairable为false。只返回JSON：{"summary":"","issues":[{"chapterId":"3","severity":"blocking|warning","type":"hardware|duplicate|diagram|test|abstract|wording","message":"","repairable":true,"find":"原文唯一片段","replace":"替换文本"}]}`,
    },
