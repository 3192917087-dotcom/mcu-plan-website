...
const API_PROVIDER_PRESETS = Object.freeze({
  deepseek: Object.freeze({ apiUrl: 'https://api.deepseek.com/chat/completions', chatModel: 'deepseek-v4-pro', reasoningModel: 'deepseek-v4-pro' }),
  zhipu: Object.freeze({ apiUrl: 'https://open.bigmodel.cn/api/paas/v4/chat/completions', chatModel: 'glm-5.2', reasoningModel: 'glm-5.2' }),
  moonshot: Object.freeze({ apiUrl: 'https://api.moonshot.cn/v1/chat/completions', chatModel: 'kimi-k3', reasoningModel: 'kimi-k3' }),
  openai: Object.freeze({ apiUrl: 'https://api.openai.com/v1/chat/completions', chatModel: 'gpt-5.2', reasoningModel: 'gpt-5.2' }),
  'openai-compatible': Object.freeze({ apiUrl: '', chatModel: '', reasoningModel: '' }),
});

const STORAGE_KEY = 'mcu-paper-studio.project.v1';
const STORAGE_BACKUP_KEY = 'mcu-paper-studio.project.backup.v1';
const OUTLINE_TEMPLATES = Object.freeze({
  balanced: Object.freeze({
    name: '标准六章结构',
    description: '硬件、软件分别成章，标题层级适中，适合大多数本科论文。',
  }),
  concise: Object.freeze({
    name: '精简二级标题结构',
    description: '尽量不用三级标题，同类器件、程序和测试集中说明。',
  }),
  layered: Object.freeze({
    name: '重点章节分层结构',
    description: '绪论和总体设计简洁，硬件、软件和测试按逻辑主题分层。',
  }),
});

const PIN_BANKS = Object.freeze({
  stm32: Object.freeze(['待确认','PA0','PA1','PA2','PA3','PA4','PA5','PA6','PA7','PA8','PA9','PA10','PA11','PA12','PA15','PB0','PB1','PB3','PB4','PB5','PB6','PB7','PB8','PB9','PB10','PB11','PB12','PB13','PB14','PB15','PC13','PC14','PC15']),
  stc: Object.freeze(['待确认','P0.0','P0.1','P0.2','P0.3','P0.4','P0.5','P0.6','P0.7','P1.0','P1.1','P1.2','P1.3','P1.4','P1.5','P1.6','P1.7','P2.0','P2.1','P2.2','P2.3','P2.4','P2.5','P2.6','P2.7','P3.0','P3.1','P3.2','P3.3','P3.4','P3.5','P3.6','P3.7']),
  esp32: Object.freeze(['待确认','GPIO2','GPIO4','GPIO5','GPIO12','GPIO13','GPIO14','GPIO15','GPIO16','GPIO17','GPIO18','GPIO19','GPIO21','GPIO22','GPIO23','GPIO25','GPIO26','GPIO27','GPIO32','GPIO33','GPIO34','GPIO35']),
  arduino: Object.freeze(['待确认','D2','D3','D4','D5','D6','D7','D8','D9','D10','D11','D12','D13','A0','A1','A2','A3','A4','A5']),
  generic: Object.freeze(['待确认']),
});
...
function loadApiSettings() {
  try {
    ...
    if (!saved || saved.mode !== 'custom') return { ...DEFAULT_API_CONFIG };
    return {
      mode: 'custom',
      provider: String(saved.provider || 'openai-compatible'),
      apiUrl: String(saved.apiUrl || '').trim(),
      apiKey: String(saved.apiKey || '').trim(),
      chatModel: String(saved.chatModel || '').trim(),
      reasoningModel: String(saved.reasoningModel || saved.chatModel || '').trim(),
    };
  } catch (error) {
...
function activeApiConfig() {
  ...
  return config.mode === 'custom' ? config : { ...DEFAULT_API_CONFIG };
}

function saveApiSettings(config) {
  if (config.mode === 'custom') localStorage.setItem(API_SETTINGS_KEY, JSON.stringify(config));
...
function schemeInputSignatureFor(value) {
  return JSON.stringify({
    ...
    functionCountMode: value?.schemeOptions?.functionCountMode || 'random',
    functionCount: Number(value?.schemeOptions?.functionCount) || 0,
...
function emptyProject() {
  ...
  return {
    ...
    schemeOptions: { functionCountMode: 'random', functionCount: 8 },
    scheme: {
      status: 'empty',
      markdown: '',
      structured: null,
...
function normalizeProject(value) {
  ...
  const normalized = {
    ...
    schemeOptions: { ...base.schemeOptions, ...(value.schemeOptions || {}) },
  ...
  normalized.materials.pinMappings = Array.isArray(normalized.materials.pinMappings)
    ? normalized.materials.pinMappings.map(item => ({
      id: item?.id || makeId('pin'),
      device: String(item?.device || '').trim(),
      connection: String(item?.connection || item?.signal || '信号线').trim(),
      pin: String(item?.pin || '待确认').trim(),
      alternatives: unique((Array.isArray(item?.alternatives) ? item.alternatives : []).map(value => String(value || '').trim()).filter(Boolean)),
    })).filter(item => item.device)
    : [];
  if (!OUTLINE_TEMPLATES[normalized.materials.outlineTemplate]) normalized.materials.outlineTemplate = 'balanced';
...
function renderSchemeBasics(panel) {
  panel.innerHTML = `
    <div class="panel-heading"><div><span class="step-kicker">第 1 步</span><h2>填写题目，就可以生成制作方案</h2><p>附加要求可以不填；一旦填写，将作为最高优先级，不会被系统默认规则覆盖。</p></div><span class="status-pill">约 1 分钟</span></div>
    <div class="scheme-entry-card material-section">
      <div class="form-grid">
        <label class="field field-wide"><span>设计题目 <b>必填</b></span><input id="scheme-title" maxlength="100" value="${escapeHtml(project.title)}" placeholder="例如：基于STM32的智能温室控制系统设计"></label>
        <label class="field field-wide"><span>附加要求 <small>选填，优先级最高</small></span><textarea id="scheme-requirements" rows="8" placeholder="可填写指定器件、必须实现的功能、使用场景、成本限制或老师要求。没有可留空。">${escapeHtml(project.requirements)}</textarea><small>示例：主控使用STM32F103C8T6；采用0.96寸OLED；需要温湿度采集、自动控制和声光报警。</small></label>
      </div>
      <fieldset class="function-count-picker">
        <legend>功能数量参考</legend>
        <label><input type="radio" name="scheme-function-count-mode" value="random" ${project.schemeOptions.functionCountMode !== 'custom' ? 'checked' : ''}><span><strong>随机数量</strong><small>根据题目在合理范围内随机安排，避免方案过于固定</small></span></label>
        <label><input type="radio" name="scheme-function-count-mode" value="custom" ${project.schemeOptions.functionCountMode === 'custom' ? 'checked' : ''}><span><strong>自定义数量</strong><small>按你填写的数量生成，不拆分重复功能</small></span></label>
        <label class="field function-count-input"><span>功能数量</span><input id="scheme-function-count" type="number" min="3" max="20" step="1" value="${Math.max(3, Number(project.schemeOptions.functionCount) || 8)}"></label>
      </fieldset>
    </div>
    <div class="notice notice-info"><strong>输出内容固定为三部分：</strong>题目、器件、功能。器件使用“型号（作用）”，功能使用“使用某器件实现某功能”的表达方式。</div>
    <div class="panel-actions"><button class="btn btn-secondary" type="button" data-action="go-home">返回首页</button><button class="btn btn-primary scheme-primary-action" type="button" data-action="submit-scheme">生成方案</button></div>`;
}
...
function captureSchemeBasics() {
  ...
  const functionCountMode = qs('input[name="scheme-function-count-mode"]:checked')?.value === 'custom' ? 'custom' : 'random';
  const requestedFunctionCount = Number($('scheme-function-count')?.value) || 8;
  if (functionCountMode === 'custom' && (requestedFunctionCount < 3 || requestedFunctionCount > 20)) {
    toast('自定义功能数量请填写3到20之间的整数', 'error');
    $('scheme-function-count')?.focus();
    return false;
  }
  const previousFacts = factsSignature();
  const previousSchemeInput = currentSchemeInputSignature();
  project.title = title;
  project.background = '';
  project.requirements = $('scheme-requirements')?.value.trim() || '';
  project.sourceMode = 'create';
  project.sourceText = '';
  project.preferences = { mcu: '', display: '', power: '' };
  project.schemeOptions = {
    functionCountMode,
    functionCount: functionCountMode === 'custom' ? Math.round(requestedFunctionCount) : 6 + Math.floor(Math.random() * 5),
  };
  const currentSchemeInput = currentSchemeInputSignature();
...
function buildSchemeDocumentText(data = schemeStructuredData()) {
  ...
  const deviceText = devices.map(item => `${item.model || item.name || item.label || '未命名器件'}  （${schemeTextValue(item.role, '作用待确认').replace(/[。；;]+$/g, '')}）`).join('，\n');
  const functionText = functions.map((item, index) => {
    const name = String(item.text || item.name || '').trim();
    const detail = schemeTextValue(item.description, name || '功能说明待确认').replace(/[。；;]+$/g, '');
    const sentence = /^使用/.test(detail) ? detail : `使用${detail}`;
...
function renderSchemeDocumentHtml() {
  ...
  return `<article class="scheme-document">
    <header><span>制作方案</span><h2>${escapeHtml(project.title || data.topic || '单片机项目方案')}</h2></header>
    <section class="scheme-document-section"><h3>器件</h3><div class="scheme-simple-devices">${devices.map(item => `<p><strong>${escapeHtml(item.model || item.name || item.label)}</strong><span>（${escapeHtml(schemeTextValue(item.role, '作用待确认').replace(/[。；;]+$/g, ''))}）</span></p>`).join('')}</div></section>
    <section class="scheme-document-section"><h3>功能</h3><ol class="scheme-function-list">${functions.map(item => { const detail = schemeTextValue(item.description, item.text || item.name || '功能说明待确认').replace(/[。；;]+$/g, ''); return `<li>${escapeHtml(/^使用/.test(detail) ? detail : `使用${detail}`)}。</li>`; }).join('')}</ol></section>
  </article>`;
}
...
function renderSchemeResultPage(panel) {
  ...
  panel.innerHTML = `
    <div class="panel-heading"><div><span class="step-kicker">第 2 步</span><h2>方案已经生成</h2><p>请整体确认器件和功能是否符合制作要求，然后复制文本或下载Word文档。</p></div><span class="status-pill ${conflicts.length ? 'is-danger' : 'is-success'}">${conflicts.length ? `${conflicts.length} 项内容待确认` : project.scheme.status === 'confirmed' ? '已确认保存' : '等待你确认'}</span></div>
    ...
    <div class="scheme-output-toolbar panel-actions"><button class="btn btn-secondary" type="button" data-action="scheme-step" data-step="1">修改并重生成</button><button class="btn btn-secondary" type="button" data-action="copy-scheme">复制方案</button><button class="btn btn-secondary" type="button" data-action="export-scheme">下载Word方案</button><button class="btn btn-primary" type="button" data-action="accept-scheme" ${conflicts.length || project.scheme.status === 'confirmed' ? 'disabled' : ''}>${project.scheme.status === 'confirmed' ? '方案已确认' : '确认方案'}</button><button class="btn btn-quiet btn-reset-project" type="button" data-action="reset-project">清空并开始下一题</button></div>`;
}
...
function schemeValidationOptions() {
  return {
    ...
    expectedFunctionCount: Number(project.schemeOptions?.functionCount) || 0,
...
async function generateScheme() {
  ...
  saveProject({ immediate: true });
  const functionTarget = Math.max(3, Math.min(20, Number(project.schemeOptions.functionCount) || 8));
  const userMessage = `【题目】${project.title}\n【最高优先级：附加要求】\n${project.requirements || '未填写'}\n【本次功能数量】${functionTarget}项\n\n请严格按题目和附加要求生成。附加要求明确指定的器件、功能和通信方式不得替换。器件输出准确型号与简短作用；每项功能必须写成“使用某器件实现某功能”的完整句子。功能不得通过同义拆分凑数量，也不得加入与题目无关的内容。`;
  requestController = new AbortController();
  requestTask = 'scheme';
  showBusy('正在生成制作方案', `正在按题目整理器件和${functionTarget}项功能`);
  try {
    ...
    let firstParsed = typeof Rules.parseSchemeResult === 'function'
      ? Rules.parseSchemeResult(raw, project.title)
      : parseSchemeFallback(raw);
    let candidate = schemeCandidateFromParsed(firstParsed);
    if (!candidate.topic || !candidate.devices.length || !candidate.functions.length) {
      throw new Error('AI没有返回完整的题目、器件和功能，请重新处理');
    }
    let validation = validateSchemeCandidate(candidate);
    let blockingValidationErrors = (validation.errors || []).filter(item => item.code !== 'scheme_unresolved_conflicts');
    for (let repairAttempt = 0; blockingValidationErrors.length && repairAttempt < 2; repairAttempt += 1) {
      updateBusyProgress(62 + repairAttempt * 16, '正在自动修正方案结构', `发现功能重复、数量或格式问题，正在进行第${repairAttempt + 1}轮修正`);
      raw = await callAi([
        { role: 'system', content: Rules.SCHEME_SYSTEM_PROMPT || '严格生成题目、器件、功能三段式单片机方案。' },
        { role: 'user', content: `${userMessage}\n\n【上一版方案】\n${raw}\n\n【必须修正的问题】\n${blockingValidationErrors.map(item => `- ${item.message}`).join('\n')}\n请完整重新输出修正后的JSON，题目与功能数量保持不变。若合并了重复功能，必须换成另一项真实、独立且与题目相关的功能，不能再次拆分同一控制链路。` },
      ], { temperature: 0.2, signal: requestController.signal });
      firstParsed = typeof Rules.parseSchemeResult === 'function' ? Rules.parseSchemeResult(raw, project.title) : parseSchemeFallback(raw);
      candidate = schemeCandidateFromParsed(firstParsed);
      validation = validateSchemeCandidate(candidate);
      blockingValidationErrors = (validation.errors || []).filter(item => item.code !== 'scheme_unresolved_conflicts');
    }
    const advisoryControlSplits = blockingValidationErrors.filter(item => item.code === 'scheme_control_chain_split');
    if (advisoryControlSplits.length) {
      validation.warnings = [...(validation.warnings || []), ...advisoryControlSplits];
      blockingValidationErrors = blockingValidationErrors.filter(item => item.code !== 'scheme_control_chain_split');
    }
    ...
    schemeStep = 2;
    renderScheme();
    toast('方案已经生成，可以复制或下载Word', 'success');
  } catch (error) {
...
function confirmScheme() {
  ...
  project.scheme.inputSignature = currentSchemeInputSignature();
  saveProject({ immediate: true });
  toast('方案已确认并保存；论文写作仍可单独开始', 'success');
  renderScheme();
}
...
function renderPaper() {
  ...
  renderStepNav($('paper-step-nav'), ['填写资料', '核对引脚', '选择目录', '生成论文'], paperStep, 'paper-step');
  for (let i = 1; i <= 5; i += 1) {
...
function controllerPinFamily(controller) {
  const value = String(controller || '').toUpperCase();
  if (/STM32|GD32|CH32|HC32/.test(value)) return 'stm32';
  if (/STC|AT89|MCS-?51|51单片机/.test(value)) return 'stc';
  if (/ESP32|ESP8266/.test(value)) return 'esp32';
  if (/ARDUINO|ATMEGA/.test(value)) return 'arduino';
  return 'generic';
}

function pinOptionsFor(controller, mapping = {}) {
  const bank = PIN_BANKS[controllerPinFamily(controller)] || PIN_BANKS.generic;
  const suggested = [mapping.pin, ...(mapping.alternatives || [])]
    .map(value => String(value || '').trim().toUpperCase())
    .filter(value => bank.includes(value));
  return unique([...suggested, ...bank]);
}

function pinConnectionText(mappings = project.materials.pinMappings) {
  return mappings
    .filter(item => item.device)
    .map(item => `${item.device}的${item.connection || '信号线'}连接主控${item.pin || '待确认'}`)
    .join('；');
}

function capturePinMappings() {
  const activePanel = $(`paper-step-${paperStep}`);
  const rows = qsa('[data-pin-mapping-id]', activePanel || document);
  if (!rows.length) return;
  project.materials.pinMappings = rows.map(row => ({
    id: row.dataset.pinMappingId || makeId('pin'),
    device: qs('[data-pin-device]', row)?.value.trim() || '',
    connection: qs('[data-pin-connection]', row)?.value.trim() || '信号线',
    pin: qs('[data-pin-select]', row)?.value || '待确认',
    alternatives: [],
  })).filter(item => item.device);
  project.materials.connectionText = pinConnectionText();
}

function renderPinMappingPanel() {
  const mappings = project.materials.pinMappings || [];
  if (!mappings.length) {
    return `<section class="pin-analysis-panel is-waiting" aria-labelledby="pin-analysis-title"><div><span class="step-kicker">硬件连接</span><h3 id="pin-analysis-title">等待器件与引脚分析</h3><p>填写题目后即可分析。器件不完整时，系统会先给出常用器件建议，再按主控整理可选择引脚。</p></div><button class="btn btn-secondary" type="button" data-action="analyze-pins">分析器件与引脚</button></section>`;
  }
  return `<section class="pin-analysis-panel" aria-labelledby="pin-analysis-title">
    <div class="pin-panel-heading"><div><span class="step-kicker">硬件连接</span><h3 id="pin-analysis-title">${escapeHtml(project.materials.pinController || '主控待确认')} 引脚选择</h3><p>下拉框只显示引脚名称。AI给出初始建议，你可以按实际原理图修改。</p></div><button class="btn btn-secondary btn-small" type="button" data-action="analyze-pins">重新分析</button></div>
    <div class="pin-mapping-list">${mappings.map(item => `<div class="pin-mapping-row" data-pin-mapping-id="${escapeHtml(item.id)}">
      <label><span>外设器件</span><input data-pin-device value="${escapeHtml(item.device)}" aria-label="外设器件名称"></label>
      <label><span>连接信号</span><input data-pin-connection value="${escapeHtml(item.connection || '信号线')}" aria-label="连接信号名称"></label>
      <label><span>主控引脚</span><select data-pin-select aria-label="选择${escapeHtml(item.device)}的主控引脚">${pinOptionsFor(project.materials.pinController, item).map(pin => `<option value="${escapeHtml(pin)}" ${pin === item.pin ? 'selected' : ''}>${escapeHtml(pin)}</option>`).join('')}</select></label>
      <button class="pin-remove-button" type="button" data-action="remove-pin-mapping" data-pin-id="${escapeHtml(item.id)}" aria-label="删除${escapeHtml(item.device)}连接">删除</button>
    </div>`).join('')}</div>
    <button class="btn btn-quiet" type="button" data-action="add-pin-mapping">＋ 添加遗漏的外设连接</button>
  </section>`;
}

async function analyzePins() {
  const title = $('paper-title')?.value.trim() || project.title;
  if (!title) {
    toast('请先填写论文题目', 'error');
    $('paper-title')?.focus();
    return;
  }
  const existingDevices = unique(lines($('paper-devices')?.value || project.materials.devicesText));
  const existingFunctions = unique(lines($('paper-functions')?.value || project.materials.functionsText));
  const existingConnectionText = $('paper-connections')?.value.trim() || project.materials.connectionText || '';
  const previousFacts = factsSignature();
  project.title = title;
  project.materials.devicesText = existingDevices.join('\n');
  project.materials.functionsText = existingFunctions.join('\n');
  project.materials.sourceNotes = $('paper-source-notes')?.value.trim() || project.materials.sourceNotes;
  project.materials.referencesText = $('paper-references')?.value.trim() || project.materials.referencesText;
  project.materials.schoolOutline = $('paper-school-outline')?.value.trim() || project.materials.schoolOutline;
  project.materials.testInfo = $('paper-test-info')?.value.trim() || project.materials.testInfo;
  project.materials.photoNotes = $('paper-photo-notes')?.value.trim() || project.materials.photoNotes;
  requestController = new AbortController();
  requestTask = 'pin-analysis';
  showBusy('正在整理器件与引脚', existingDevices.length ? '正在识别主控、外设和连接信号' : '当前只有题目，正在先补齐常用器件和功能建议');
  try {
    const raw = await callAi([
      { role: 'system', content: `你是单片机项目硬件连接整理助手。用户已有内容优先，不得替换。器件或功能为空时，根据题目给出一套常见、现实、适合本科制作的建议。识别主控后，将每个外设需要连接主控的信号逐行列出；电源和地不放入可选GPIO列表。推荐引脚必须与主控封装和常见复用关系相容，不能重复占用同一引脚。只返回JSON：{"controller":"准确主控型号","devices":["准确器件型号"],"functions":["功能"],"mappings":[{"device":"外设型号","connection":"时钟线/数据线/控制线/模拟量输出等","pin":"PA1","alternatives":["PA2"]}],"fixedFacts":["可靠通信常识"],"conflicts":["只有用户资料确实矛盾时填写"]}` },
      { role: 'user', content: JSON.stringify({ title, devices: existingDevices, functions: existingFunctions, confirmedConnections: existingConnectionText || '未提供', notes: project.materials.sourceNotes || '未提供' }, null, 2) },
    ], { temperature: 0.1, maxTokens: 10000, model: 'thinking', jsonMode: true, signal: requestController.signal });
    const result = parseJsonResponse(raw);
    const suggestedDevices = normalizedFactList(result.devices);
    const suggestedFunctions = normalizedFactList(result.functions);
    const controller = String(result.controller || [...existingDevices, ...suggestedDevices].find(isControllerFact) || '').trim();
    const pinBank = PIN_BANKS[controllerPinFamily(controller)] || PIN_BANKS.generic;
    const mappings = (Array.isArray(result.mappings) ? result.mappings : []).map(item => {
      const requestedPin = String(item?.pin || '待确认').trim().toUpperCase();
      return {
        id: makeId('pin'),
        device: String(item?.device || '').trim(),
        connection: String(item?.connection || item?.signal || '信号线').trim(),
        pin: pinBank.includes(requestedPin) ? requestedPin : '待确认',
        alternatives: normalizedFactList(item?.alternatives)
          .map(value => String(value).trim().toUpperCase())
          .filter(value => pinBank.includes(value)),
      };
    }).filter(item => item.device && !isControllerFact(item.device));
    project.materials.devicesText = unique([
      controller,
      ...(existingDevices.length ? existingDevices : suggestedDevices),
    ].filter(Boolean)).join('\n');
    project.materials.functionsText = (existingFunctions.length ? existingFunctions : suggestedFunctions).join('\n');
    project.materials.pinController = controller;
    project.materials.pinMappings = mappings;
    project.materials.pinStatus = 'ready';
    project.materials.connectionText = existingConnectionText || pinConnectionText(mappings);
    invalidateIfFactsChanged(previousFacts);
    const conflicts = normalizedFactList(result.conflicts);
    project.audit = {
      ...project.audit,
      status: 'reviewing',
      summary: conflicts.length ? '发现需要核对的资料冲突，请确认后继续。' : '器件、功能和引脚建议已整理，请按实际原理图核对下拉框。',
      issues: conflicts.map(detail => normalizeIssue({ severity: 'blocking', title: '资料存在冲突', detail })),
      factSheet: normalizeFactSheet({ recommendedDevices: suggestedDevices, coreDevices: suggestedDevices, recommendedFunctions: suggestedFunctions, coreFunctions: suggestedFunctions, fixedFacts: result.fixedFacts }),
      confirmationSignature: '',
      confirmedAt: '',
      inputRevision: project.revision,
    };
    saveProject({ immediate: true });
    renderPaper();
    toast(mappings.length ? '引脚建议已生成，请按实际连接核对' : '器件与功能已整理，请补充实际引脚后继续', 'success');
  } catch (error) {
    if (error.name !== 'AbortError') toast(error.message || '器件与引脚分析失败', 'error');
  } finally {
    hideBusy();
    requestController = null;
    requestTask = '';
...
function renderPaperMaterials(panel) {
  ...
  panel.innerHTML = `
    <div class="panel-heading"><div><span class="step-kicker">第 1 步</span><h2>把现在有的资料放进来</h2><p>只有论文题目也能开始。器件、功能、程序和参考文献越完整，生成内容越贴合实际项目。</p></div><span class="status-pill">${usingScheme ? '已带入网站方案' : '论文独立使用'}</span></div>
    <div class="notice notice-info"><strong>方案与论文互不绑定。</strong> 你可以直接在这里写自己的方案资料；${canImportScheme ? '也可以选择带入刚才确认的网站方案。' : '没有使用方案功能也不影响论文生成。'}</div>
    ${canImportScheme ? `<div class="panel-actions compact-actions"><button class="btn btn-secondary" type="button" data-action="use-scheme-for-paper">带入网站方案（可继续修改）</button></div>` : ''}

    <section class="material-section">
      <div class="material-section-heading"><div><span>01</span><h3>题目与项目说明</h3></div><small>题目必填，其余选填</small></div>
      <div class="form-grid">
        <label class="field field-wide"><span>论文题目 <b>必填</b></span><input id="paper-title" value="${escapeHtml(project.title)}" placeholder="例如：基于STM32的智能鱼缸控制系统设计"></label>
        <label class="field field-wide"><span>完整方案、任务书或补充说明 <small>选填</small></span><textarea id="paper-source-notes" rows="9" placeholder="有资料就完整粘贴；没有可以留空。这里的信息优先于系统推测。">${escapeHtml(materials.sourceNotes)}</textarea></label>
      </div>
    </section>

    <section class="material-section">
      <div class="material-section-heading"><div><span>02</span><h3>器件、功能与引脚</h3></div><small>不确定可以留空后分析</small></div>
      <div class="form-grid">
        <label class="field"><span>器件清单 <small>一行一个</small></span><textarea id="paper-devices" rows="10" placeholder="例如：\nSTM32F103C8T6\n0.96寸OLED\nDHT11">${escapeHtml(materials.devicesText)}</textarea></label>
        <label class="field"><span>功能要求 <small>一行一个</small></span><textarea id="paper-functions" rows="10" placeholder="例如：\n温湿度采集\n数据显示\n超限报警">${escapeHtml(materials.functionsText)}</textarea></label>
      </div>
      ${renderPinMappingPanel()}
      ${materials.pinMappings.length ? `<label class="field field-wide connection-summary-field"><span>整理后的硬件连接 <small>可继续手动修正</small></span><textarea id="paper-connections" rows="6" placeholder="分析引脚后自动生成；也可以粘贴自己的原理图连接关系。">${escapeHtml(materials.connectionText)}</textarea></label>` : ''}
    </section>

    <section class="material-section">
      <div class="material-section-heading"><div><span>03</span><h3>程序与参考文献</h3></div><small>均为选填</small></div>
      <div class="form-grid">
        <div class="field"><span>单片机源程序</span><label class="folder-upload-card" for="paper-code-files"><strong>选择代码文件夹</strong><small>只读取文件夹中的 .c 和 .h 文件</small><input id="paper-code-files" type="file" accept=".c,.h" webkitdirectory directory multiple hidden></label><div class="selected-file-summary">${materials.filenames.length ? `已读取 ${materials.filenames.length} 个文件：${escapeHtml(materials.filenames.slice(0, 5).join('、'))}${materials.filenames.length > 5 ? '…' : ''}` : '尚未选择程序文件夹'}</div><textarea id="paper-code" class="code-source-store" hidden>${escapeHtml(materials.codeText)}</textarea></div>
        <label class="field"><span>参考文献 <small>使用 [x] 区分每一篇</small></span><textarea id="paper-references" rows="12" placeholder="[x] 作者. 题名[J]. 期刊名, 年份, 卷(期): 页码.\n摘要：……\n\n[x] 作者. 题名[D]. 城市: 学位授予单位, 年份.\n摘要：……">${escapeHtml(materials.referencesText)}</textarea><small>粘贴后只使用你提供的文献；正文按首次引用顺序生成[1][2][3]，文末参考文献同步重新排序。</small></label>
      </div>
    </section>

    <details class="material-more-details">
      <summary>更多资料：测试记录、实物图片、学校目录</summary>
      <div class="form-grid">
        <label class="field"><span>测试、调试和操作记录 <small>选填</small></span><textarea id="paper-test-info" rows="8" placeholder="没有实测数据也可以留空，系统会生成保守、可修改的量化测试表。">${escapeHtml(materials.testInfo)}</textarea></label>
        <label class="field"><span>实物照片与待插图说明 <small>选填</small></span><textarea id="paper-photo-notes" rows="8" placeholder="说明有哪些实物图、功能展示图；图片后续在WPS中插入。">${escapeHtml(materials.photoNotes)}</textarea></label>
        <label class="field field-wide"><span>学校目录或往届论文目录 <small>选填，优先采用</small></span><textarea id="paper-school-outline" rows="10" placeholder="有就粘贴，没有可在下一步选择网站目录结构。">${escapeHtml(materials.schoolOutline)}</textarea></label>
      </div>
    </details>
    <div class="panel-actions"><button class="btn btn-secondary" type="button" data-action="go-home">返回首页</button><button class="btn btn-primary" type="button" data-action="save-paper-materials">保存资料，核对引脚</button></div>`;
}
...
function capturePaperMaterials() {
  ...
  project.materials = {
    ...
    connectionText: $('paper-connections')?.value.trim() || project.materials.connectionText || '',
...
function renderFactAudit(panel) {
  ...
  panel.innerHTML = `
    <div class="panel-heading"><div><span class="step-kicker">第 2 步</span><h2>只需要确认器件和引脚是否符合实物</h2><p>器件分配、功能归纳和常见连接由系统整理。你只需修改不符合实际的型号或下拉引脚。</p></div><span class="status-pill ${blocking ? 'is-danger' : project.audit.status === 'reviewing' ? 'is-success' : ''}">${blocking ? `${blocking} 项冲突` : project.audit.status === 'reviewing' ? '等待确认' : '等待分析'}</span></div>
    <div class="paper-fact-summary"><div><span>论文题目</span><strong>${escapeHtml(project.title || '未填写')}</strong></div><div><span>主控</span><strong>${escapeHtml(project.materials.pinController || paperDevices().find(isControllerFact) || '等待分析')}</strong></div><div><span>器件 / 功能</span><strong>${paperDevices().length} / ${paperFunctions().length}</strong></div></div>
    ${project.audit.summary ? `<div class="notice notice-info">${escapeHtml(project.audit.summary)}</div>` : ''}
    ${renderPinMappingPanel()}
    ${issues.length ? `<div class="issue-list">${issues.map(renderIssueCard).join('')}</div>` : ''}
    <div class="panel-actions"><button class="btn btn-secondary" type="button" data-action="paper-step" data-step="1">返回修改资料</button><button class="btn btn-secondary" type="button" data-action="analyze-pins">${project.audit.status === 'reviewing' ? '重新分析器件与引脚' : '分析器件与引脚'}</button>${project.audit.status === 'reviewing' ? '<button class="btn btn-primary" type="button" data-action="confirm-facts">确认无误，选择目录</button>' : ''}</div>`;
}
...
function conciseOutline() {
  return `第1章 绪论\n1.1 研究背景及意义\n1.2 国内外研究现状\n1.3 本文主要研究内容\n1.4 论文组织结构\n\n第2章 系统总体方案设计\n2.1 系统需求分析\n2.2 系统总体架构\n2.3 系统功能设计\n2.4 主要器件选型\n2.5 系统方案确定\n\n第3章 系统硬件设计\n3.1 硬件总体设计\n3.2 主控最小系统与电源设计\n3.3 信息采集电路设计\n3.4 显示、通信与执行电路设计\n3.5 硬件抗干扰设计\n\n第4章 系统软件设计\n4.1 软件开发环境与总体架构\n4.2 数据采集与处理流程\n4.3 显示、通信与人机交互逻辑\n4.4 控制、报警与联动逻辑\n4.5 软件异常处理\n\n第5章 系统调试与功能测试\n5.1 开发调试环境\n5.2 硬件与软件调试\n5.3 系统功能测试\n5.4 测试结果分析\n\n第6章 总结与展望`;
}

function layeredOutline() {
  return `第1章 绪论\n1.1 研究背景及意义\n1.2 国内外研究现状\n1.2.1 国内研究现状\n1.2.2 国外研究现状\n1.2.3 研究现状综合分析\n1.3 主要研究内容与论文结构\n\n第2章 系统需求与总体设计\n2.1 应用需求分析\n2.2 系统总体架构\n2.3 系统工作过程\n2.4 主控及外围器件选型\n2.5 总体方案确定\n\n第3章 系统硬件设计\n3.1 硬件总体结构\n3.2 主控最小系统与供电电路\n3.3 传感与信号采集电路\n3.3.1 数字量采集电路\n3.3.2 模拟量采集电路\n3.4 显示、通信与执行电路\n3.4.1 显示与人机交互电路\n3.4.2 通信与执行驱动电路\n\n第4章 系统软件设计\n4.1 软件总体架构\n4.2 系统主流程设计\n4.3 数据采集与处理逻辑\n4.4 显示、通信与控制逻辑\n4.4.1 数据显示与通信流程\n4.4.2 控制、报警与联动流程\n4.5 软件异常处理\n\n第5章 系统调试与功能测试\n5.1 系统调试环境与方法\n5.2 硬件和软件联合调试\n5.3 基础功能测试\n5.4 综合功能与稳定性测试\n5.4.1 综合联动功能测试\n5.4.2 稳定性与响应性能测试\n5.5 测试结果分析\n\n第6章 总结与展望`;
}

function outlineTemplateText(key = project.materials.outlineTemplate) {
  if (key === 'concise') return conciseOutline();
  if (key === 'layered') return layeredOutline();
  return fallbackOutline();
}

function buildDefaultOutline() {
  if (project.materials.schoolOutline) return project.materials.schoolOutline;
  if (project.materials.outlineTemplate && project.materials.outlineTemplate !== 'balanced') return outlineTemplateText(project.materials.outlineTemplate);
...
function renderOutline(panel) {
  ...
  const selectedTemplate = project.materials.outlineTemplate || 'balanced';
  panel.innerHTML = `
    <div class="panel-heading"><div><span class="step-kicker">第 3 步</span><h2>选择适合学校习惯的目录结构</h2><p>三个模板不是方向差异，而是标题层级和内容组织方式不同。选中后仍可直接修改。</p></div><span class="status-pill">正文不少于 18,000 字</span></div>
    ${project.materials.schoolOutline ? '<div class="notice notice-info"><strong>已提供学校目录：</strong>当前优先采用你粘贴的结构。选择模板会替换编辑框内容，但不会删除原始资料。</div>' : ''}
    <div class="outline-template-panel" role="radiogroup" aria-label="论文目录结构">
      ${Object.entries(OUTLINE_TEMPLATES).map(([key, template]) => `<label class="outline-template-card"><input type="radio" name="outline-template" value="${key}" data-outline-template ${selectedTemplate === key ? 'checked' : ''}><span><strong>${escapeHtml(template.name)}</strong><small>${escapeHtml(template.description)}</small><code>${key === 'balanced' ? '2.4 主要器件选型\n2.4.1 主控制器选型' : key === 'concise' ? '3.3 信息采集电路设计\n3.4 显示、通信与执行电路设计' : '3.3 传感与信号采集电路\n3.3.1 数字量采集电路'}</code></span></label>`).join('')}
    </div>
    ...
    <div class="form-grid compact"><label class="field"><span>正文目标字数</span><input id="paper-target" type="number" min="18000" step="1000" value="${Math.max(18000, Number(project.paper.targetChars) || 20000)}"><small>系统会根据项目难度自动分配到各章节，最低不会少于18,000字。</small></label></div>
    <div class="panel-actions"><button class="btn btn-secondary" type="button" data-action="paper-step" data-step="2">返回核对引脚</button><button class="btn btn-secondary" type="button" data-action="reset-outline">恢复当前模板</button><button class="btn btn-primary" type="button" data-action="confirm-outline">确认目录，准备生成</button></div>`;
}
...
function chapterTitleFromOutline(id) {
  const line = String(project.outline.text || '').split(/\r?\n/)
    .map(value => value.trim().replace(/^#{1,6}\s*/, ''))
    .find(value => new RegExp(`^第\\s*${id}\\s*章`).test(value));
  return line?.replace(new RegExp(`^第\\s*${id}\\s*章\\s*`), '').trim() || CHAPTER_TITLES[id];
}
...
function ensureChapterRecords() {
  ...
  for (let id = 1; id <= 6; id += 1) {
    ...
    Object.assign(existing, {
      id: key,
      title: chapterTitleFromOutline(id),
...
async function runSemanticAuditBatch(batch, signal) {
  ...
  const signatureSource = JSON.stringify({
    ...
    model: activeApiConfig().reasoningModel || activeApiConfig().chatModel,
...
async function callAi(messages, { temperature = 0.4, maxTokens = 8192, model = 'normal', jsonMode = false, signal, configOverride = null } = {}) {
  const apiConfig = configOverride || activeApiConfig();
  if (!apiConfig.apiUrl || !apiConfig.apiKey || !apiConfig.chatModel) throw makeAiError('当前生成服务设置不完整，请在右上角“设置”中检查', { code: 'auth' });
  const reasoning = model === 'thinking' || model === 'deepseek-reasoner';
  const isDeepSeek = apiConfig.provider === 'deepseek' || /api\.deepseek\.com/i.test(apiConfig.apiUrl);
  const payload = {
    model: reasoning ? (apiConfig.reasoningModel || apiConfig.chatModel) : apiConfig.chatModel,
    messages,
    stream: false,
  ...
  } else if (apiConfig.provider === 'moonshot') {
    payload.temperature = 1;
  } else if (apiConfig.provider === 'zhipu') {
    payload.temperature = 1;
    if (reasoning) payload.thinking = { type: 'enabled' };
  } else if (apiConfig.provider === 'openai') {
    delete payload.max_tokens;
    payload.max_completion_tokens = Math.min(Math.max(1, Number(maxTokens) || 8192), 65536);
  } else if (!reasoning && Number.isFinite(temperature)) {
...
function apiConfigFromForm() {
  const mode = qs('input[name="api-mode"]:checked')?.value === 'custom' ? 'custom' : 'default';
  if (mode === 'default') return { ...DEFAULT_API_CONFIG };
  return {
    mode: 'custom',
    provider: $('api-provider')?.value || 'openai-compatible',
    apiUrl: $('api-base-url')?.value.trim() || '',
    apiKey: $('api-key')?.value.trim() || '',
    chatModel: $('api-chat-model')?.value.trim() || '',
    reasoningModel: $('api-reasoning-model')?.value.trim() || $('api-chat-model')?.value.trim() || '',
  };
}

function apiConfigError(config) {
  if (config.mode !== 'custom') return DEFAULT_API_CONFIG.apiKey ? '' : '默认API尚未配置。';
  if (!config.apiUrl) return '请填写API地址。';
  if (!/^https?:\/\//i.test(config.apiUrl)) return 'API地址需要以 http:// 或 https:// 开头。';
  if (!config.apiKey) return '请填写API Key。';
  if (!config.chatModel) return '请填写普通模型名称。';
  return '';
}

function setApiSettingsError(message = '') {
  const node = $('api-settings-error');
  if (!node) return;
  node.textContent = message;
  node.hidden = !message;
}

function toggleApiCustomFields() {
  const custom = qs('input[name="api-mode"]:checked')?.value === 'custom';
  const fields = $('api-custom-fields');
  if (fields) fields.hidden = !custom;
  if (custom && !$('api-base-url')?.value.trim() && !$('api-chat-model')?.value.trim()) {
...
function renderApiSettingsForm() {
  const config = loadApiSettings();
  qsa('input[name="api-mode"]').forEach(input => { input.checked = input.value === config.mode; });
  if ($('api-provider')) $('api-provider').value = config.provider || 'openai-compatible';
  if ($('api-base-url')) $('api-base-url').value = config.mode === 'custom' ? config.apiUrl : '';
  if ($('api-key')) { $('api-key').value = config.mode === 'custom' ? config.apiKey : ''; $('api-key').type = 'password'; }
  if ($('btn-toggle-api-key')) $('btn-toggle-api-key').textContent = '显示';
  if ($('api-chat-model')) $('api-chat-model').value = config.mode === 'custom' ? config.chatModel : '';
  if ($('api-reasoning-model')) $('api-reasoning-model').value = config.mode === 'custom' ? config.reasoningModel : '';
  toggleApiCustomFields();
  checkService();
}

function openApiSettings() {
  renderApiSettingsForm();
  const dialog = $('dialog-api-settings');
  if (dialog?.showModal) dialog.showModal();
}

function applyApiProviderPreset() {
  const preset = API_PROVIDER_PRESETS[$('api-provider')?.value] || API_PROVIDER_PRESETS['openai-compatible'];
  if ($('api-base-url')) $('api-base-url').value = preset.apiUrl;
  if ($('api-chat-model')) $('api-chat-model').value = preset.chatModel;
  if ($('api-reasoning-model')) $('api-reasoning-model').value = preset.reasoningModel || preset.chatModel;
  setApiSettingsError('');
}

function submitApiSettings() {
  const config = apiConfigFromForm();
  const error = apiConfigError(config);
  if (error) {
    setApiSettingsError(error);
    return false;
  }
  saveApiSettings(config);
  checkService();
  closeDialog('dialog-api-settings');
  toast(config.mode === 'custom' ? '自定义API已保存并启用' : '已继续使用默认API', 'success');
  return true;
}

function resetApiSettings() {
  saveApiSettings({ ...DEFAULT_API_CONFIG });
  renderApiSettingsForm();
  toast('已恢复默认API，普通使用无需其他设置', 'success');
}

function markServiceVerified() {
  const node = $('service-status');
  if (!node) return;
  node.innerHTML = '<span aria-hidden="true"></span> 连接正常，可以生成内容';
...
async function testApiConnection() {
  ...
  const config = $('dialog-api-settings')?.open ? apiConfigFromForm() : activeApiConfig();
  const configProblem = apiConfigError(config);
  if (configProblem) {
    setApiSettingsError(configProblem);
    return;
  }
  if (button) {
    button.disabled = true;
  ...
  try {
    ...
    ], { temperature: 0, maxTokens: 128, configOverride: config });
    setApiSettingsError('');
    toast('连接测试成功，内容生成功能可以使用', 'success');
  } catch (error) {
    const node = $('service-status');
    if (node) {
    ...
    setApiSettingsError(`连接失败：${error.message}`);
...
async function handleFileInput(input) {
  ...
  try {
    ...
    if (input.id === 'paper-code-files') {
      const codeFiles = files.filter(file => /\.(?:c|h)$/i.test(file.name));
      if (!codeFiles.length) throw new Error('所选文件夹中没有找到 .c 或 .h 文件');
      const previousFacts = factsSignature();
      const parts = [];
      for (const file of codeFiles) {
        const text = await readTextFile(file);
        parts.push(`\n/* 文件：${file.webkitRelativePath || file.name} */\n${text}`);
      }
      ...
      project.materials.filenames = unique([...project.materials.filenames, ...codeFiles.map(file => file.webkitRelativePath || file.name)]);
      invalidateIfFactsChanged(previousFacts);
      saveProject({ immediate: true });
      toast(`已读取 ${codeFiles.length} 个 C/H 程序文件`, 'success');
    }
  } catch (error) {
...
function bindEvents() {
  document.addEventListener('click', event => {
    ...
    if (action === 'analyze-pins') analyzePins();
    if (action === 'add-pin-mapping') {
      capturePinMappings();
      project.materials.pinMappings.push({ id: makeId('pin'), device: '新增外设', connection: '信号线', pin: '待确认', alternatives: [] });
      project.materials.pinStatus = 'ready';
      saveProject({ immediate: true });
      renderPaper();
    }
    if (action === 'remove-pin-mapping') {
      capturePinMappings();
      project.materials.pinMappings = project.materials.pinMappings.filter(item => item.id !== button.dataset.pinId);
      project.materials.connectionText = pinConnectionText();
      saveProject({ immediate: true });
      renderPaper();
    }
    ...
    if (action === 'reset-outline') { project.outline.text = project.materials.schoolOutline || outlineTemplateText(project.materials.outlineTemplate); renderPaper(); }
  ...
  document.addEventListener('change', event => {
    ...
    if (input.matches('[data-pin-select]')) { capturePinMappings(); saveProject(); }
    if (input.matches('[data-outline-template]')) {
      project.materials.outlineTemplate = input.value;
      project.outline.text = outlineTemplateText(input.value);
      project.outline.confirmedAt = '';
      saveProject({ immediate: true });
      renderPaper();
    }
  ...
  document.addEventListener('input', event => {
    ...
    if (event.target.matches('[data-pin-device], [data-pin-connection]')) capturePinMappings();
  });
  ...
  $('btn-close-api-settings')?.addEventListener('click', () => closeDialog('dialog-api-settings'));
  qsa('input[name="api-mode"]').forEach(input => input.addEventListener('change', toggleApiCustomFields));
  $('api-provider')?.addEventListener('change', applyApiProviderPreset);
  $('btn-toggle-api-key')?.addEventListener('click', () => {
    const input = $('api-key');
    if (!input) return;
    input.type = input.type === 'password' ? 'text' : 'password';
    $('btn-toggle-api-key').textContent = input.type === 'password' ? '显示' : '隐藏';
  });
  $('btn-reset-api-settings')?.addEventListener('click', resetApiSettings);
  $('form-api-settings')?.addEventListener('submit', event => {
    event.preventDefault();
    submitApiSettings();
  });
...
async function checkService() {
  ...
  if (node) {
    ...
    const configured = Boolean(config.apiUrl && config.apiKey && config.chatModel);
    node.innerHTML = `<span aria-hidden="true"></span> ${configured ? (config.mode === 'custom' ? '自定义API已启用' : '默认API已就绪') : 'API未配置'}`;
