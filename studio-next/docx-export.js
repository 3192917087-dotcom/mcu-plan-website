...
function headingParagraph(text, level, { pageBreakBefore = false } = {}) {
  ...
  return new Paragraph({
    ...
    keepNext: true,
    spacing: level === 1
      ? { before: 0, after: 240, line: BODY_LINE, lineRule: LineRuleType.AUTO }
      : { before: level === 2 ? 240 : 120, after: level === 2 ? 120 : 60, line: BODY_LINE, lineRule: LineRuleType.AUTO },
    children: inlineRuns(text, { font: FONT_HEADING, size, bold: true }),
  });
}
...
function bodyParagraph(text, options = {}) {
  return new Paragraph({
    ...
    indent: options.noIndent ? undefined : { firstLine: BODY_FIRST_LINE },
...
function annotationParagraph(text) {
  return new Paragraph({
    ...
    shading: { fill: 'F2F2F2' },
    children: [bodyRun(text, { size: 21, color: '666666' })],
  });
}
  ...
  const tableRows = rows.map((row, rowIndex) => new TableRow({
    ...
    children: widths.map((width, cellIndex) => new TableCell({
      ...
      children: [new Paragraph({
        ...
        children: inlineRuns(row[cellIndex] || '', { size: 21, bold: rowIndex === 0 }),
      })],
    })),
  }));
  return new Table({
...
function frontHeading(text, { english = false, pageBreakBefore = false } = {}) {
  return new Paragraph({
    ...
    pageBreakBefore,
    keepNext: true,
    spacing: { before: 0, after: 240, line: BODY_LINE, lineRule: LineRuleType.AUTO },
    children: [bodyRun(text, { font: english ? FONT_LATIN : FONT_HEADING, size: 32, bold: true })],
  });
}

function keywordParagraph(keywords) {
  return new Paragraph({
    alignment: AlignmentType.LEFT,
    spacing: { before: 240, after: 0, line: BODY_LINE, lineRule: LineRuleType.AUTO },
    children: [
      bodyRun('关键词：', { font: FONT_HEADING, bold: true }),
      bodyRun(keywords),
    ],
  });
}
  ...
  return {
    default: {
      document: {
        ...
        paragraph: { alignment: AlignmentType.JUSTIFIED, spacing: { before: 0, after: 0, line: BODY_LINE, lineRule: LineRuleType.AUTO } },
      },
      heading1: {
...
async function buildPaperDocx(input) {
  ...
  const frontChildren = [
    ...
    frontHeading('Abstract', { english: true, pageBreakBefore: true }),
